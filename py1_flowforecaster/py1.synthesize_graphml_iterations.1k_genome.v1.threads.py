...
def synthesize_iterations(filename: str, iterations: int):
    """
    data:
    ...
    * id has extension [".vcf", ".gz", ".txt"]
    ...
    for iteration in range(iterations - 1):
        ...
        for src, dst, attr in G.edges(data=True):
            ...
            new_dst = map_nodes_old_to_new[dst]
            new_edge_attr = generate_edge_attr_random()
            new_edges_list.append((new_src, new_dst, new_edge_attr))
    ...
    print(f"\nNew graph with iterations {iterations}:")
    ...
    return {
        "G": new_G,
        "taskid_dict": taskid_dict,
        "fileid_dict": fileid_dict
    }


def synthesize_caterpillars(threads: int,
                            G,
                            taskid_dict: dict,
                            fileid_dict: dict):
    if threads < 2:
        print(f"Number of threads is {threads}, no need to synthesize threads. Returned.")
        return G
    ...
    for _ in range(threads - 1):
        ...
        for src, dst, attr in G.edges(data=True):
            ...
            new_dst = map_nodes_old_to_new[dst]
            new_edge_attr = generate_edge_attr_random()
            new_G.add_edge(new_src, new_dst, **new_edge_attr)

    print(f"\nNew graph with threads {threads}:")
    # pprint(list(new_G.nodes))
    print(f"num_nodes: {new_G.number_of_nodes()} num_edges: {new_G.number_of_edges()}")

    # # test
    # show_dag(new_G, msg=f"synthesize-{threads}")
    # # end test
...
if __name__ == "__main__":
    ...
    parser.add_argument("-t", "--threads", type=int, default=2, help="number of caterpillars (threads) to synthesize")
    ...
    iter_result = synthesize_iterations(filename=graphml_file, iterations=iterations)
    graph_thrds = synthesize_caterpillars(threads=threads, **iter_result)

    basename = os.path.splitext(os.path.basename(graphml_file))[0]
    new_filename = basename + ".iter-" + str(iterations) + ".thrd-" + str(threads) + ".graphml"
    nx.write_graphml(graph_thrds, new_filename)
    print(f"Saved the new graph to {new_filename}")
